...
export default function DashboardPage() {
  ...
  if (isLoading) return <div className="h-screen bg-[#060608] flex items-center justify-center text-white/10 font-mono text-[10px] uppercase tracking-[0.4em]">Initialising...</div>;
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-14 shrink-0 flex items-center justify-between px-6 border-b border-white/[0.06]">
          ...
          <div className="flex items-center gap-3">
             ...
             <button onClick={() => setShowModal(true)} className="text-[11px] tracking-[0.15em] uppercase bg-white text-[#060608] px-4 py-2 rounded-lg font-bold hover:bg-white/90 transition-colors font-mono">+ Add Monitor</button>
          </div>
        </header>

        <div className="flex-1 overflow-auto p-6">
          {page === "dashboard" && <DashboardView count={competitors.length} signals={signals} />}
          {page === "competitors" && <CompetitorsView competitors={competitors} onDelete={handleDelete} />}
          ...
          <div className="bg-[#0f1012] border border-white/10 rounded-[32px] p-8 w-full max-w-md shadow-2xl relative">
            ...
            <form onSubmit={handleAddCompetitor} className="space-y-5">
              <div className="relative" ref={companyRef}>
                <p className="text-[10px] font-mono text-white/20 uppercase mb-2 text-[9px] tracking-widest">Company / ИП</p>
                ...
                }} placeholder="БУКЕТ-СБ..." className="w-full bg-white/[0.03] border border-white/[0.08] rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-white/30 transition-all font-mono"/>
              ...
              <div className="relative" ref={cityRef}>
                <p className="text-[10px] font-mono text-white/20 uppercase mb-2 text-[9px] tracking-widest">Primary Location</p>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-4 gap-4">
        {[{ label: "Monitors", value: count }, { label: "Total Signals", value: signals.length }, { label: "Score", value: "84" }, { label: "Status", value: "Online" }].map((s, i) => (
      ...
      <div className="grid grid-cols-3 gap-4">
          <div className="col-span-2 border border-white/[0.07] rounded-[32px] p-20 text-center bg-white/[0.01]">
            <p className="text-white/20 text-xs font-mono uppercase tracking-widest">Waiting for signal spikes...</p>
          </div>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
      {competitors.map((c: any) => (
        <div key={c.id} className="border border-white/[0.07] rounded-2xl p-5 bg-white/[0.02] hover:border-white/15 transition-all group relative">
          <div className="flex items-center gap-3 mb-4">
            ...
            <button onClick={() => onDelete(c.id)} className="p-2 text-white/10 hover:text-red-400 transition-colors">
              {Icons.trash}
            </button>
          </div>
