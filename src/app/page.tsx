  ...
  <div className="w-full h-32 flex items-end gap-1 px-2">
    {[40, 60, 45, 90, 65, 80, 50, 40, 70, 85, 100, 75, 60, 55, 90, 110, 80, 60].map((h, i) => (
      <motion.div
        ...
        className="flex-1 bg-white/20 hover:bg-white/50 transition-colors rounded-t-sm"
      />
    ))}
  </div>
);
...
const SignalRow = ({ company, action, time, type }: any) => (
  <motion.div 
    ...
    viewport={{ once: true }}
    className="grid grid-cols-4 py-4 border-b border-white/5 text-[11px] hover:bg-white/[0.02] transition-colors px-4 items-center"
  >
...
const SignalRadar = () => {
  ...
  return (
    <div className="w-full aspect-[16/9] bg-black rounded-3xl overflow-hidden border border-white/10 relative flex flex-col">
      <div className="flex items-center justify-between px-5 py-3.5 border-b border-white/[0.06] bg-black z-10 shrink-0">
        ...
        <span className="text-[9px] font-mono text-white/15">STATUS: ONLINE</span>
      </div>

      <div className="flex-1 grid grid-cols-3 gap-px bg-white/[0.04] relative overflow-hidden">
        {COLUMNS.map((col, ci) => (
          <div key={ci} className={`${ci === 1 ? "bg-[#050505]" : "bg-black"} p-4 flex flex-col gap-2.5 overflow-hidden relative`}>
      ...
      <div className="border-t border-white/[0.05] overflow-hidden py-2 bg-black shrink-0">
        ...
        >
          {doubled.map((item, i) => (
            <span key={i} className="text-[9px] font-mono text-white/20 tracking-[0.15em]">
              {item}
            </span>
          ))}
        </motion.div>
...
export default function SledixApp() {
  ...
  return (
    <div className="bg-black text-white min-h-screen font-sans selection:bg-white selection:text-black antialiased">
        ...
        <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2 cursor-pointer" onClick={() => window.scrollTo({top: 0, behavior: 'smooth'})}>
              <SledixLogo />
              <span className="text-sm font-bold uppercase tracking-tighter">Sledix</span>
            </div>
          ...
          <div className="flex items-center gap-4">
            <button 
              ...
              className="text-[10px] font-mono text-zinc-500 hover:text-white border border-white/10 px-2 py-1 rounded transition-colors"
            >
              {lang}
            </button>
            <button className="bg-white text-black px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest hover:bg-zinc-200 transition-all">
              {t.cta_primary}
            </button>
          </div>
        </div>
      ...
      {/* Hero Section */}
      <section id="hero" className="pt-40 pb-20 px-6 max-w-6xl mx-auto border-x border-white/5">
        <div className="max-w-3xl">
          <motion.div 
            initial={{ opacity: 0 }} 
            animate={{ opacity: 1 }} 
            className="inline-flex items-center gap-2 px-2 py-1 rounded border border-white/10 text-[9px] font-bold uppercase tracking-[0.2em] text-zinc-500 mb-8"
          >
          ...
          <motion.h1 
            ...
            viewport={{ once: true }}
            className="text-6xl md:text-8xl font-bold tracking-tighter uppercase leading-[0.85] mb-10"
          >
            {t.hero_title}
          </motion.h1>
          <motion.p 
            ...
            className="text-base md:text-xl text-zinc-400 font-medium max-w-lg leading-snug mb-12"
          >
            {t.hero_sub}
          </motion.p>

          <div className="flex gap-4 flex-wrap">
            <button className="bg-white text-black px-8 py-4 rounded-full text-[11px] font-bold uppercase tracking-widest hover:scale-105 transition-all flex items-center gap-2">
              {t.cta_primary} <ArrowUpRight size={14} />
            </button>
            <button
              onClick={() => window.location.href = '/about'}
              className="border border-white/30 text-white px-8 py-4 rounded-full text-[11px] font-bold uppercase tracking-widest hover:scale-105 hover:border-white transition-all flex items-center gap-2"
            >
        ...
        <motion.div 
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="mt-32 border border-white/10 rounded-2xl bg-[#080808] overflow-hidden shadow-2xl"
        >
          <div className="p-6 border-b border-white/10 flex justify-between items-center bg-white/[0.02]">
            <div className="flex items-center gap-3">
              <Activity size={16} className="text-zinc-500" />
              <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">{t.chart_title}</span>
            </div>
          ...
          <div className="border-t border-white/10">
            <div className="bg-white/5 px-6 py-3 flex justify-between items-center">
               <span className="text-[9px] font-bold uppercase tracking-widest text-zinc-500">{t.signals_title}</span>
            </div>
            <SignalRow company="Notion" action="Pricing Strategy Pivot" type="FINANCE" time="12:04" />
            <SignalRow company="Linear" action="Enterprise Sales Expansion" type="HR" time="11:42" />
            <SignalRow company="Stripe" action="Checkout ToS Update" type="LEGAL" time="10:15" />
          </div>
        </motion.div>
      </section>

      {/* Technology Section Anchor */}
      <section id="tech" className="px-6 max-w-6xl mx-auto border-x border-white/5 grid md:grid-cols-3 gap-px ">
        {t.features.map((f, i) => (
          <motion.div 
            key={i} 
            initial={{ opacity: 0 }}
            ...
            className="bg-black p-10 py-20 flex flex-col gap-6 group hover:bg-zinc-950 transition-colors"
          >
            {i === 0 ? <Search size={22} className="text-zinc-700 group-hover:text-white transition-colors" /> : 
             i === 1 ? <Lock size={22} className="text-zinc-700 group-hover:text-white transition-colors" /> : 
             <Sliders size={22} className="text-zinc-700 group-hover:text-white transition-colors" />}
            <h3 className="text-xs font-black uppercase tracking-widest italic">{f.t}</h3>
            <p className="text-[11px] text-zinc-500 leading-relaxed uppercase tracking-widest">{f.d}</p>
          </motion.div>
        ))}
      </section>

      {/* Pricing Section Anchor */}
      <section id="pricing" className="py-40 px-6 max-w-6xl mx-auto border-x border-white/5">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-20 gap-6">
          <h2 className="text-5xl font-bold uppercase tracking-tighter italic leading-none">{t.pricing_title}</h2>
          <span className="text-[10px] font-bold text-zinc-600 uppercase tracking-[0.4em]">
            {t.founding_member}
          </span>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {t.plans.map((tier, i) => (
            <motion.div 
              key={i}
              whileHover={{ y: -10 }}
              className={`p-10 rounded-3xl border transition-all duration-100 ${tier.active ? 'border-white bg-white text-black scale-[1.05] z-10' : 'border-white/10 bg-black hover:border-white/30'}`}
            >
              <div className="flex justify-between items-start mb-20">
                <span className="text-[11px] font-black uppercase tracking-widest italic opacity-50">{tier.n}</span>
                <span className="text-4xl font-bold tracking-tighter italic">${tier.p}</span>
              </div>
              <ul className="space-y-5 mb-20">
                {tier.feat.map((f, idx) => (
                  <li key={idx} className="text-[10px] font-bold uppercase tracking-widest flex items-center gap-3">
                    <ShieldCheck size={14} className={tier.active ? "text-black" : "text-zinc-800"} /> {f}
                  </li>
                ))}
              </ul>
              <button className={`w-full py-5 rounded-full text-[10px] font-bold uppercase tracking-[0.3em] transition-all ${tier.active ? 'bg-black text-white hover:bg-zinc-900' : 'bg-white text-black hover:bg-zinc-200'}`}>
                {t.cta_primary}
              </button>
            </motion.div>
          ))}
      ...
      <section className="py-20 px-6 max-w-6xl mx-auto border-x border-white/5">
        <div className="max-w-xl mb-12">
          <h2 className="text-3xl font-bold uppercase tracking-tighter italic mb-4">{t.scanner_title}</h2>
      ...
      {/* Footer */}
      <footer id="about" className="py-10 px-6 max-w-6xl mx-auto border-t border-white/5 flex flex-col items-center">
        <h2 className="text-6xl md:text-9xl font-black uppercase italic tracking-tighter text-center mb-16 leading-[1]">
          {t.footer_header_1} <br /> <span className="text-zinc-800">{t.footer_header_2}.</span>
        </h2>

        <form action="https://formspree.io/f/mpqoongy" method="POST" className="w-full max-w-md flex flex-col gap-8">
          <input 
            type="email" 
            name="email"
            required
            placeholder={t.waitlist_placeholder} 
            className="bg-transparent border-b border-white/10 py-6 text-center text-base font-bold uppercase focus:border-white outline-none transition-all placeholder:text-zinc-900"
          />
          <button type="submit" className="text-[11px] font-black uppercase tracking-[0.6em] text-zinc-600 hover:text-white transition-colors">
             {t.waitlist_btn}
          </button>
        </form>

        <div className="mt-48 w-full flex flex-col md:flex-row justify-between items-center gap-8 text-[9px] font-bold text-zinc-800 uppercase tracking-[0.5em]">
           <span>{t.rights}</span>
